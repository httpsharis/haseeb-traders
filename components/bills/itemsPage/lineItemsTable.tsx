...
export function LineItemsTable() {
  ...
  useEffect(() => {
    fetch("/api/categories")
      ...
      .then((cats) => {
        ...
        const activeCats = catArray.filter(
          (c: Category) => c.isActive !== false,
        );

        setCategories(activeCats);

        // If we didn't find an existing category (brand new bill), pick the first one
  ...
  // FIX: Removed 'any' and replaced with strict Typescript Generics
  const updateField = <K extends keyof LineItem>(
    id: string,
    field: K,
    value: LineItem[K],
  ) => {
  ...
  const inputClasses =
    "w-full bg-transparent border border-transparent hover:border-slate-200 focus:border-[#ea580c] focus:bg-white rounded-md px-2 py-1.5 text-sm text-slate-900 font-medium outline-none transition-all";
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative group w-full sm:w-64">
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <svg
                className="h-4 w-4 text-slate-400 group-focus-within:text-[#ea580c] transition-colors"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead className="bg-white border-b border-slate-200">
            <tr>
              <th className="py-3 px-4 text-xs font-bold text-slate-400 uppercase tracking-wider w-12">
                SR
              </th>
              <th className="py-3 px-2 text-xs font-bold text-slate-400 uppercase tracking-wider">
                Description
              </th>
              <th className="py-3 px-2 text-xs font-bold text-slate-400 uppercase tracking-wider text-right w-24">
                Qty
              </th>
              <th className="py-3 px-2 text-xs font-bold text-slate-400 uppercase tracking-wider text-right w-32">
                Price
              </th>
              <th className="py-3 px-2 text-xs font-bold text-orange-600 uppercase tracking-wider text-right w-24">
                GST %
              </th>
              <th className="py-3 px-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right w-32">
                Total
              </th>
      ...
      >
        ...
        {globalCategory
          ...
          : "Select a category above to start adding items"}
      </button>
    </div>
  );
}
